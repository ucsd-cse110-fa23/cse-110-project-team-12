...
public class US8Test {
    ...
    public void testUS8BDD1() {
        ...
        assertEquals(CreateRecipeModel.PageType.IngredientsInput,
                ((CreateRecipeModel) controller.getState(Controller.ModelType.CreateRecipe)).getCurrentPage());
        controller.receiveMessageFromUI(new Message(Message.CreateRecipeView.RecordButton));
        controller.receiveMessageFromUI(new Message(Message.CreateRecipeView.RecordButton)); // Given I am waiting for
                                                                                             // the recipe to be created
                                                                                             // -> When the Recipe is
                                                                                             // created

        // Then I am taken to the detailed view for the recipe
        assertEquals(Controller.UIType.DetailedView,
                ((HomeModel) controller.getState(Controller.ModelType.HomePage)).getCurrentView());

        assertNotEquals("",
                ((RecipeDetailedModel) controller.getState(Controller.ModelType.DetailedView)).getRecipeTitle());
        assertNotEquals("",
                ((RecipeDetailedModel) controller.getState(Controller.ModelType.DetailedView)).getRecipeBody());
    }
